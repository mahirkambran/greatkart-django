...
def payments(request):
    ...
    payment = Payment(
        ...
        amount_paid = order.order_total,  # Mock amount
        status = body['status']
    )
...
def place_order(request, total=0, quantity=0):
    ...
    # 🟢 Handle GET requests — just show order form
    else:
        form = OrderForm()
        return render(request, 'orders/place_order.html', {
            'form': form,
...
def order_complete(request):
        ...
        transID = request.GET.get('payment_id') or request.GET.get('transID')
